...
def render_calendar():
    with st.expander("📅 달력", expanded=True):
        ...
        with ct1:
            ...
            with n1:
                if st.button("◀", key="cp", use_container_width=True):
                    ...
                    st.rerun()
            with n2:
                st.markdown(f'<div style="text-align:right;font-size:1.1rem;font-weight:700;color:var(--black);padding:0.3rem 0;">{st.session_state.cal_year}년 {st.session_state.cal_month}월</div>', unsafe_allow_html=True)
            with n3:
        ...
        with ct2:
            ...
            st.markdown(f'<div style="text-align:center;font-size:1rem;font-weight:700;color:var(--black);margin-bottom:0.5rem;">{mon.strftime("%m/%d")} — {sun.strftime("%m/%d")} 이번 주</div>', unsafe_allow_html=True)
